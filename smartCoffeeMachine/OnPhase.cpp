...
bool OnPhase::updateAndCheckTime(int basePeriod){
  if(GLOBAL_CLASS.getActualPhase() == EnumPhase::ON_ASCENT || GLOBAL_CLASS.getActualPhase() == EnumPhase::ON_DISCENT){ // se la fase attuale e' ON qualsiasi
    ...
    if(timeElapsed <= (GLOBAL_CLASS.getActualPhase() == EnumPhase::ON_ASCENT ? myPeriodAscent : myperiodDiscent)){ //se la fase attuale e' onAscent utilizza dt1 altrimenti dt2b
        return true;
...
void OnPhase::tick(){
    if(sonar->getValue() > GLOBAL_CLASS.getDist1()) { //se non rilevo nessuno sotto i 30 cm torno in SLEEP
